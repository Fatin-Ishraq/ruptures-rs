...
No library changes: every detector, cost and result is identical to 0.1.0. This
is a documentation release, and it goes out as a release because PyPI fixes a
project's description at upload time — the page cannot be corrected without one.

### Changed

- **The README now explains what the library is for before how fast it is.** It
  opened on a benchmark and a migration diff, which reads well if you already
  use `ruptures` and tells a stranger nothing: it never said what change point
  detection *is*, how it differs from anomaly detection, or where anyone would
  use it. It now starts there, and 3,158 words became 1,180.
- The specialist material moved to `docs/` rather than being cut, since it is
  worth having: [`benchmarks.md`](docs/benchmarks.md),
  [`accuracy.md`](docs/accuracy.md), [`differences.md`](docs/differences.md).
- Three figures, generated by `assets/make_assets.py`: the fitted model over a
  noisy signal, a log-scale chart of the measured speedups, and the CROPS
  penalty path. The speedup chart reads `bench/results.json` at generation
  time, so it cannot drift from the table it illustrates.
- Two README claims were wrong and are now measured. The hero figure said the
  fit found every change point "exactly"; it misses one boundary by twenty
  samples out of two thousand, because the level change there is small next to
  the noise. The quick-start example's breakpoints and timing were invented.
- Source distributions no longer carry `assets/` or `bench/`, which is half a
  megabyte of images that nobody building from source will look at. The sdist
  is 91 KB.
